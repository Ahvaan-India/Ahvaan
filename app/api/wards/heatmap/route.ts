      ...
      async () => {
        ...
        const wards = board.wards.map((w) => {
          ...
          return {
            ...
            wbgt: w.wbgt ?? r?.indicators.wbgt ?? null,
            heatIndex: w.heatIndex ?? r?.indicators.heatIndex ?? null,
            utci: w.utci ?? r?.indicators.utci ?? null,
            population: w.totalPopulation,
            thermal: w.thermal ?? r?.scores.thermalStress ?? null,
            exposure: w.exposure ?? r?.scores.exposure ?? null,
            vulnerability: w.vulnerability ?? r?.scores.vulnerability ?? null,
            temp: w.temp ?? null,
            humidity: w.humidity ?? null,
            wind: w.wind ?? null,
            solar: w.solar ?? null,
            realFeel: w.realFeel ?? null,
            elderlyPct: w.elderlyPct ?? null,
            childrenPct: w.childrenPct ?? null,
            outdoorWorkerPct: w.outdoorWorkerPct ?? null,
            informalIndex: w.informalIndex ?? null,
    ...
    try {
      ...
      const fallbackWards = Array.from({ length: 144 }, (_, i) => {
        ...
        return {
          ...
          category: htsiVal >= 65 ? "VERY_HIGH" : htsiVal >= 50 ? "HIGH" : htsiVal >= 30 ? "MODERATE" : "LOW",
          displayCategory: htsiVal >= 65 ? "Extreme" : htsiVal >= 50 ? "High" : htsiVal >= 30 ? "Moderate" : "Low",
          step: htsiVal >= 80 ? 5 : htsiVal >= 65 ? 4 : htsiVal >= 50 ? 3 : htsiVal >= 30 ? 2 : 1,
