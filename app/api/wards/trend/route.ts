...
export async function GET(req: Request) {
  ...
  try {
    ...
    const rows = await db.execute<{ date: string; avgRisk: number; count: number }>(sql`
      SELECT
    ...
    const raw: Array<{ date: string; avgRisk: number; count: number }> =
      (rows as unknown as { rows?: typeof rows })?.rows ?? (rows as unknown as Array<{ date: string; avgRisk: number; count: number }>);
    const sorted = [...(Array.isArray(raw) ? raw : [])].sort((a, b) => (a.date < b.date ? -1 : 1));
    return NextResponse.json({ days: sorted }, { headers: { "Cache-Control": "public, s-maxage=300, stale-while-revalidate=120" } });
  } catch (err) {
