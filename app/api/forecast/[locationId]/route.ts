...
export async function GET(req: Request, { params }: RouteParams) {
  ...
  const days = Number.isInteger(daysRaw) ? Math.max(1, Math.min(daysRaw, 10)) : 5;

  try {
    const location = await getLocationById(locationId);
    if (!location) throw new LocationNotFoundError(locationId);
    const population = await getLatestPopulation(locationId);
    if (!population) throw new DataGapError(locationId, "no population/census row");
    ...
    if (rows.length === 0) throw new DataGapError(locationId, "no usable weather rows in range");

    const forecastDays = computeForecastDays({ location, population, rows, timeZone, now, days });
    ...
    if (err instanceof DataGapError) {
      return NextResponse.json(
        { error: "Insufficient data for forecast", locationId, detail: err.detail },
        { status: 422 },
      );
    }
